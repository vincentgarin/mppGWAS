...
#'
#' @param haplo.geno \code{gpData} haplotype \code{character matrix}
#' with haplotype block in rows and genotype in columns. Row i specify the
#' haplotype code for each genotype. This object can be obtained using the
#' function \code{haplo_blocks()}.
#'
...
#' containing the list of markers that will be used to compute the kinship
#' matrix.
#'
...
# haplo.geno <- haplo.data[[1]]
# haplo.map <- haplo.data[[2]]
# haplo.term <- "fixed"
# trait <- pheno[, 1]
# gp <- gp.imp
# map <- map
...
######################### end checks
  ...
  # 2.2 gen, map and nalleles

  haplo.scan.data <- gen.map.nalleles(haplo.geno = haplo.geno,
                                      haplo.map = haplo.map)

  gen.sc <- haplo.scan.data[[1]]
  map.sc <- haplo.scan.data[[2]]
  nalleles <- haplo.scan.data[[3]]
  ...
  if(!K_i){ # Use the whole genome for K.
    ...
    scan <- magicScan_mod(dataframe = d, gen = gen.sc, map = map.sc,
                          kk.eigen = kk.eigen, nalleles = nalleles,
                          model = model)

    res <- lapply(1:length(map.sc), function(i){ return(scan[[i]]) })
    ...
    for(i in 1:n.chr){
      ...
      scan <- magicScan_mod(dataframe = d, gen = gen.sc[i], map = map.sc[i],
                            kk.eigen = kk.eigen, nalleles = nalleles[i],
                            model = model)
